	...
	jobs: test: {
		strategy: {
			...
			matrix: {
				...
				runner: "${{ ((! \(_repo.containsDispatchTrailer)) && (github.repository == '\(_repo.githubRepositoryPath)')) && fromJSON(\(strconv.Quote(encjson.Marshal([_repo.linuxMachine, _repo.macosMachine])))) || fromJSON(\(strconv.Quote(encjson.Marshal([_repo.linuxMachine])))) }}"
			}
		}
