	...
	jobs: {
		test: {
			...
			steps: [
				...
				_netlifyDeploy & {
					let trybotTrailerExpr = "fromJSON(steps.TryBotTrailer.outputs.value)"
					if:     "github.repository == '\(_repo.trybotRepositoryPath)' && \(_repo.containsSpecialTrailer & {#trailer: _repo.trybot.trailer, _})"
					#site:  _repo.netlifySites.cls
					#alias: "cl-${{ \(trybotTrailerExpr).CL }}-${{ \(trybotTrailerExpr).patchset }}"
