			...
			steps: [
				...
				json.#step & {
					run: """
						td=$(mktemp -d)
						mkdir $td/.banana
						cp README.md $td/.banana
						env -i /usr/local/bin/docker run --network=host -e USER_UID=501 -e USER_GID=20 --rm -i -v $td/.banana/README.md:/tmp/README.md golang ls -la /tmp
						env -i /usr/local/bin/docker run --network=host -e USER_UID=501 -e USER_GID=20 --rm -i -v $td/.banana/README.md:/tmp/README.md golang cat /tmp/README.md
						"""
				},
