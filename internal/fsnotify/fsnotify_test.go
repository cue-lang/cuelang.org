...
Walk:
	...
	if _gittoplevel != nil {
		...
		run(gittoplevel, "git", "init")
		run(gittoplevel, "git", "config", "user.name", "blah")
		run(gittoplevel, "git", "config", "user.email", "blah@blah.com")
		run(gittoplevel, "git", "add", "-A")
		run(gittoplevel, "git", "commit", "-m", "initial commit")
	}
...
func run(dir, cmd string, args ...string) {
	initCmd := exec.Command(cmd, args...)
	initCmd.Dir = dir
	byts, err := initCmd.CombinedOutput()
	if err != nil {
