...
func (s setupCtx) watcher() error {
	w, err := fsnotify.NewWatcher(s.rootdir, s.gittoplevel, fsnotify.Debug(os.Stderr))
	if err != nil {
...
func (s setupCtx) batchedWatcher(d time.Duration) error {
	w, err := fsnotify.NewBatchedWatcher(s.rootdir, s.gittoplevel, d, fsnotify.Debug(os.Stderr))
	if err != nil {
		return fmt.Errorf("failed to create a Watcher: %w", err)
	}
	s.e.Defer(func() {
