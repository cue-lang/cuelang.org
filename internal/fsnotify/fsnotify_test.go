...
const (
	...
	batchedWatcherHandlerKey = "batchedWatcherHandler"
...
func (s setupCtx) debugOpt() fsnotify.Option {
	if *fDebug {
...
func (s setupCtx) watcher() error {
	w, err := fsnotify.NewRecursiveWatcher(s.rootdir, s.debugOpt())
	if err != nil {
	...
	s.e.Values[batchedWatcherHandlerKey] = bwh
	go bwh.run()
	return nil
}

func (s setupCtx) batchedWatcher(d time.Duration) error {
	bw, err := fsnotify.NewBatchedRecursiveWatcher(s.rootdir, s.gittoplevel, d, s.debugOpt())
	if err != nil {
		return fmt.Errorf("failed to create a Watcher: %w", err)
	}
	s.e.Defer(func() {
...
func handleEvent(b *batchedWatcherHandler[fsnotify.Event], specialFile string, ev fsnotify.Event) string {
	...
	if err != nil {
		b.s.l.logf("error: failed to derive %q relative to %q: %v", ev.Name, b.s.rootdir, err)
	} else {
		b.s.l.logf("name: %s, op: %v\n", rel, ev.Op)
	}
...
func logCmd(ts *testscript.TestScript, neg bool, args []string) {
	...
	bwh, ok := ts.Value(batchedWatcherHandlerKey).(specialHander)
	if !ok {
	...
	if !ok {
		ts.Fatalf("failed to find watcher log")
	}
	snapshot, err := tw.snapshot()
	if err != nil {
