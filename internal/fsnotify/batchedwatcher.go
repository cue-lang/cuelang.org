...
// NewBatchedWatcher creates a new batched watcher. A BatchedWatcher's Events
// are batched into windows of at most batchWindow duration. Furthermore, if
// gittoplevel is supplied, events received during git operations are batched
// until the end of the git operation. This can mean that batch time windows
// can be less than batchWindow when a git operation starts before a time
// window expires. It can also mean that a batch captures events over a time
// period greater than batchWindow, when a git operation exceeds this duration.
