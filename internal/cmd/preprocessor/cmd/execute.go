...
var (
	...
	// preprocessor.
	supportedLanguages = []lang{langEn, langDe}
...
//
// Args is a list of possibly absolute directory paths to filter.
...
func (ec *executeContext) newPage(dir, rel string) (*page, error) {
	// Load a CUE package in dir
	// cfg := &load.Config{
	// 	Dir: dir,
	// }
	// bps := load.Instances([]string{"."}, cfg)
	// v := e.ctx.BuildInstance(bps[0])
	// if err := v.Validate(); err != nil {
	// 	return nil, fmt.Errorf("failed to load CUE package: %w", err)
	// }
...
func (p *page) process() error {
	...
	for _, lang := range langs {
		...
		for _, rootFile := range rootFiles {
			...
			sourcePath := filepath.Join(p.dir, prefix+string(lang)+"."+ext)
			targetDir := filepath.Join(p.ctx.executor.root, "hugo", "content", string(lang), p.relPath)
			targetPath := filepath.Join(targetDir, prefix+"index."+ext)

			if err := p.processRootFile(rootFile, sourcePath, targetPath); err != nil {
				return err
			}
		}
	}
