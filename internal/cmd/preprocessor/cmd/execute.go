...
const (
	...
	flagAllowCacheMiss flagName = "allowcachemiss"
...
type executionContext struct {
	...
	// allowCacheMiss is indicates whether a cache hit is required or not for
...
func executeDef(c *Command, args []string) error {
	...
	if flagSkipCache.Bool(c) && !flagAllowCacheMiss.Bool(c) {
		return fmt.Errorf("cannot use --skipcache and --allowcachemiss=false together")
	}
	...
	ctx := executionContext{
		...
		allowCacheMiss:    flagAllowCacheMiss.Bool(c),
		ctx:               cuectx,
