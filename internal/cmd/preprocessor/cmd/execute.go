...
func executeDef(c *Command, args []string) error {
	...
	var testUserAuthnFn func() (map[string]wireToken, error)
	if authFlag := flagTestUserAuthn.String(c); authFlag != "" {
		...
		testUserAuthnFn = sync.OnceValues(func() (res map[string]wireToken, err error) {
