...
func newPage(e *executor, dir, rel string) *page {
	return &page{
		exec:        e,
		dir:         dir,
		langTargets: make(map[string][]string),
	...
	for _, langDir := range langDirs {
		...
		err := filepath.WalkDir(langDir, func(path string, d os.DirEntry, err error) error {
			...
			if err := os.MkdirAll(td, 0777); err != nil {
				return fmt.Errorf("failed to ensure target directory %s exists: %w", td, err)
			}
			sf, err := os.Open(path)
			if err != nil {
			...
			if err != nil {
				return fmt.Errorf("failed to create target file %s: %w", tfn, err)
			}
			if _, err := io.Copy(tf, sf); err != nil {
				tf.Close()
				return fmt.Errorf("failed to copy %s to %s: %w", path, tfn, err)
			}
			if err := tf.Close(); err != nil {
				return fmt.Errorf("failed to close %s after copy: %w", tfn, err)
			}
			return nil
		})
		if err != nil {
