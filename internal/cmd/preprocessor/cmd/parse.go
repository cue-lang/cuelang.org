	...
	for _, n := range ln.Nodes {
		...
		case *parse.WithNode:
			...
			if err == nil {
				...
				if prev != nil {
					prevText = prev.(*textNode)
				}
				if newNode.isHidden() && prevText != nil {
					if bytes.HasSuffix(prevText.transformText, []byte("\n")) {
						prevText.transformText = bytes.TrimRight(prevText.transformText, "\n")
						// Add back one
						prevText.transformText = append(prevText.transformText, '\n')
					}
				}
			}
