...
func (s *sidebysideNodeRunContext) run() (err error) {
	...
	if !isEffectiveComment(effectiveArchive.Comment) {
		...
		if !ok {
			s.debugf("%v: failed to build effective comment", s)
			return
		}
	}
...
func (s sidebysideNodeRunContext) buildEffectiveScript(ar *txtar.Archive) ([]byte, bool) {
	...
	if len(files) == 2 && files[0].Basename == "in" && files[1].Basename == "out" {
		...
		case "cue", "json", "yaml":
		default:
			s.errorf("%v: unknown input extension %q on %s", s, in.Ext, in.Filepath)
		}
		switch out.Ext {
		case "cue", "json", "yaml":
		default:
			s.errorf("%v: unknown output extension %q on %s", s, out.Ext, out.Filepath)
		}
		type args struct {
			In  filenameAnalysis
		...
		exec cue export --out {{.Out.Ext}} {{.In.Ext}}: {{.In.Filepath}}
		cmp stdout {{.Out.Filepath}}
		`, args{In: in, Out: out})
