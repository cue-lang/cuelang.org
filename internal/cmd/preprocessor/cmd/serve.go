...
func (e *executor) serve(args []string) error {
	...
	// Serve
	sc.w, err = fsnotify.NewRecursiveBatchedWatcher(e.wd, sc.gittoplevel, 10*time.Millisecond, oDebug)
	if err != nil {
...
func (sc *serveContext) waitOnHugo() {
	if err := sc.hugo.Wait(); err != nil {
		sc.errs <- fmt.Errorf("got a non-zero exit code from [%v]: %v\n", sc.hugo, err)
	} else {
		sc.errs <- nil
	}
}
