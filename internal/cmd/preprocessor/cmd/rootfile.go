...
func (m *multiStepScript) run() (runerr error) {
	...
	// changed.
	if len(m.page.config.UserAuthn) > 0 {
		authMap, err := m.userAuthn()
		if err != nil {
			m.fatalf("%v: failed to load authn map: %v", m, err)
		}
		users := make(map[string]bool)
		for _, u := range m.page.config.UserAuthn {
			users[u] = true
		}
		var vars []string
		for u := range users {
			...
			vars = append(vars, fmt.Sprintf("USER_AUTHN_%s=%s", safeUser, creds.AccessToken))
		}
		if m.isInError() {
