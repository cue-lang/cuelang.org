...
func (rf *rootFile) buildMultistepScript() (runnable, error) {
	...
	rf.walkBody(func(nn node) error {
		switch n := nn.(type) {
		case *scriptNode:
			didWork = true
...
func (m *multiStepScript) run() (runerr error) {
	...
	var args []string
	args = append(args,
		// We use docker create followed by docker start to avoid problems with stdin, stdout and stderr
		// not being attached to a real tty. Not quite sure what happens to make this "work" but it does,
		// a lesson learned from preguide days.
		"docker", "create", "--rm",
	...
	m.walkBody(func(step node) error {
