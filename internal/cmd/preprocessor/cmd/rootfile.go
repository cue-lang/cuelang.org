...
func (rf *rootFile) buildMultistepScript() (*multiStepScript, error) {
	...
	for _, k := range slices.Sorted(maps.Keys(rf.baseEnv)) {
		pf("export %s='%s'\n", k, strings.ReplaceAll(rf.baseEnv[k], "'", "'\\''"))
	}
...
func (m *multiStepScript) run() {
	...
	args = append(args,
		...
		"--mount", fmt.Sprintf("type=bind,source=%s,target=/scripts,readonly,z", scriptsDir),
	...
	m.walkBody(func(n node) error {
		...
		case *uploadNode:
			// We only have work to do here if we have an assert directive and are
			// in --update mode. In which case we need to update the contents of
			// the upload node, which is the "want" in the comparison.
