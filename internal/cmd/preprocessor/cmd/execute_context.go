...
dirs:
	for _, absDir := range ec.order {
		...
		for _, e := range entries {
			n := e.Name()
			if filepath.Ext(n) == ".cue" {
				filenames = append(filenames, filepath.Join(absDir, n))
			}
		}
		...
		// We now want to walk down into v to ensure that
		// the only fields that exist at each "level" are c
		// consistent with the elements of path
		...
		for _, elem := range parts {
			...
			for fieldIter.Next() {
				...
				if sel.LabelType() != cue.StringLabel || sel.Unquoted() != elem {
					...
					pos := val.Pos()
					ec.errorf("%v:%d: %v: field not allowed", pos.Filename(), pos.Line(), badPath)
				}
			}
			if inError {
