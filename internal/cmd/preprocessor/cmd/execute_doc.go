...
func newExecuteCmd(c *Command) *cobra.Command {
	...
	cmd.Flags().StringSliceVar(&debugArgs, string(flagDebug), nil, "turn on debug logging. Comma-separated (negatable) list of: 'general', 'fsnotify', 'cache', 'hugo'. 'all' is a special category that encompasses all debug logging. The --debug flag can appear multiple times.")
	...
	cmd.Flags().Bool(string(flagUpdate), os.Getenv(envUpdate) != "", "update files in archives when formatting and running scripts. Can also be set with non-empty "+envUpdate+" env var.")
	...
	cmd.Flags().Bool(string(flagPrintDockerImage), false, "print the docker image tag and exit")
	return cmd
}
