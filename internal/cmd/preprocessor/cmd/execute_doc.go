...
func newExecuteCmd(c *Command) *cobra.Command {
	...
	cmd.Flags().StringSliceVar(&debugArgs, string(flagDebug), nil, "turn on debug logging. Comma-separated (negated) list of: 'general', 'fsnotify', 'cache', 'hugo'. 'all' is a special category that encompasses all debug logging. The --debug flag can appear multiple times.")
