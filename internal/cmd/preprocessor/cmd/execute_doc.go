...
// /content directory.
//
// Note that using page bundles me the URL path now has a trailing slash:
//
//	https://alpha.cuelang.org/docs/howto/validate-yaml-using-cue/
//
...
//
// The existence of a single ${LANG}.md or _${LANG}.md file in a directory
// marks a page root. In the context of processing a page we will refer to this
// as the $SOURCE_DIR.
//
...
//
// Note this is not recursive. It only applies to the files in the guide
// directory.
//
// If the source directory contains a specially named directory _${LANG} then
// the contents of that directory and recursively copied to the correspond The
// execute command is responsible for sourcing /content and transforming it
// into /hugo/content, respecting the directory and file structures described
// above.
//
// Implicit in the above therefore is that directories starting with _ (underscore)
// are ignored when we walk the working directory.
