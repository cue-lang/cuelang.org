...
type localContext struct {
	...
	DispatchTrailerMode string `json:",omitempty"`
...
func (c *localContext) setCLandPatchsetFromDispatchTrailer(run *github.WorkflowRun) bool {
	if c.DispatchTrailerMode == modeIgnore {
		return false
	}
	...
	if dt == "" {
		...
		if c.DispatchTrailerMode == modeRequire {
...
func (fn Function) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	c.DispatchTrailerMode = os.Getenv(EnvDispatchTrailerMode)
	switch c.DispatchTrailerMode {
	case modeIgnore, modePrefer, modeRequire:
	default:
		c.DispatchTrailerMode = modeIgnore
	}
