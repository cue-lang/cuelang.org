...
host:   *"localhost:80" | =~"[-_a-zA-Z0-9]+" @tag(host)
scheme: *"https" | "http"                    @tag(scheme)

command: checkEndpoints: redirections: {

	clientSide: {for e in hugoAliases {
		let Url = "\(scheme)://\(host)\(e.path)"
		(Url): http.Get & {
			url: Url
			response: {
				let Content = #"<meta http-equiv=refresh content="0; url=\#(scheme)://\#(host)\#(e.redirection)">"#
				statusCode: >=200 & <300
...
command: checkEndpoints: content: {
	"go get cuelang.org/go": {
		let Url = "\(scheme)://\(host)/go/\(dummyPath)?go-get=1"
