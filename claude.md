...
**Important:** The `Preprocessor-No-Write-Cache: true` must be added as a proper git trailer (after `Signed-off-by` and `Change-Id` lines), not as part of the commit message body.

### Editing Commits in a Stack

When you need to edit a commit that's part of a commit stack (multiple commits on a branch), always use the branchpoint for interactive rebase:
...
Where `N` is the line number of the commit you want to edit in the rebase todo list (counting from 1).
...
This ensures you're rebasing from the correct base and can safely edit commits without breaking the commit stack or losing work.