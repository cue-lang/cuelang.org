...
`latest` version of CUE stored in `site.cue`.
...
It's intended that this site should have the exact same template and content as
[cuelang.org](https://cuelang.org), except that all pages will have their data
formatted and output emitted by `cuelang.org/go@master`.

We intend that any commit to `master` on
`cue-lang/cuelang.org` or [github.com/cue-lang/cue](https://github.com/cue-lang/cue)
will result in a redeploy of [tip.cuelang.org](https://tip.cuelang.org),
