...
Replace directives require CUE v0.17.0 or later.
...
permitted in `module.cue` at all: placing one there is an error that directs you
to use `local-module.cue` instead.
...
flags, writing them to `local-module.cue` and creating or removing that file as
needed. [`cue mod tidy`]({{<relref "docs/reference/command/cue-help-mod-tidy">}})
preserves replace directives in `local-module.cue` while recording the
published dependency information in `module.cue`. For a worked example, see
[Replacing a dependency with a local directory]({{<relref "docs/howto/replace-a-dependency-with-a-local-directory">}}).
