...
commutative, idempotent, and recusive. This means that when values are unified,
CUE guarantees that every possible order in which they *might* be combined
produces the same underlying data structure, and therefore it doesn't matter
which *specific* ordering is chosen - even when unifying deeply nested data.
...
In the case of the value of `B`, you'd be right - this is simply a
demonstration of explict unification so that you can recognise it when we use
it later, in more interesting situations.
...
CUE doesn't distinguish between values and types.
