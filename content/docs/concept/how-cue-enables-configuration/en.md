...
Here's an example of `cue vet` checking `aSpec.yaml` against a `#Spec` that
contains some of the types and constraints CUE can encode, along with the
...
- `values.yml`: contains detail of an app being deployed
...
and exports a JSON file for the deployment process to use.
...
concept merges the handling of basic types, concrete data, and the constraints
that apply to values.
The
[order-independent]({{< relref "docs/tour/basics/order-irrelevance" >}})
[unification]({{< relref "docs/tour/basics/duplicate-fields/" >}})
model removes the need for confusing, overlapping defaults.
Instead, CUE's powerful
[constraints]({{< relref "docs/tour/basics/constraints" >}}) unify,
permitting schema and policy elements simply to state the values and limits
that they need to enforce.
CUE does the hard work of checking that all the elements are compatible, and
that any configuration data meets the combined requirements.
