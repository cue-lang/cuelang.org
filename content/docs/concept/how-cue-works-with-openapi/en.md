...
The `cue` command can convert CUE schemas into OpenAPI's "components" data
schema form. CUE files can be converted into OpenAPI so long as they only
specify definitions and metadata (`info`, `$version`, etc) at their top-level.
...
Be aware of how *long* an equivalent OpenAPI definition can become - not all
formats possess CUE's succinctness and economy of expression!
