...
- we manage a pair of applications, `alpha` and `beta`
- we run them on an orchestration system that needs non-CUE input
...
CUE ensures that wherever we remove a default value from `alpha.cue` or `beta.cue`,
...
- we **cannot** set it as a default value that allows overriding if a future app needs to, due to cue-trim's behaviour
  - validate behaviour with Matthew, and maybe open an issue.
...
The orchestration system's YAML now includes the `gamma` app, with our
...
An export confirms that our orchestration system's YAML still contains all the
