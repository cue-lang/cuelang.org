...
{{{with _script "en" "HIDDEN_ set up caches"}}}
export LC_ALL=C
...
{{{with _script "en" "HIDDEN_ cue get go - reset"}}}
rm -rf $HOME/*
...
Here, we're going to fetch some Go source code published by the Kubernetes project,
import some types it defines, and show some of the CUE that gets produced.

Let's start with a specific version of the k8s.io/api module:
...
that we add will be checked against the associated Kubernetes type's schema:
...
{{{with _script "en" "HIDDEN_ load cue - reset"}}}
rm -rf $HOME/*
go mod init cue.example
{{{end}}}
<!-- syntax highlighting hack* -->
...
{{{with _script "en" "HIDDEN_ load non-cue - reset"}}}
rm -rf $HOME/*
go mod init cue.example
{{{end}}}
<!-- syntax highlighting hack* -->
...
{{{with _script "en" "HIDDEN_ check go data - reset"}}}
rm -rf $HOME/*
go mod init cue.example
{{{end}}}
<!-- syntax highlighting hack* -->
