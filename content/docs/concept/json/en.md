...
against CUE's powerful and compact syntax, it's easy to use CUE to add
"pre-flight" checks to an existing process.

In this example, `cue` is used to check that a hypothetical system's JSON input
files adhere to certain constraints, and catches a problematic deployment early
in the process:
...
-- config-b.json --
{
    ...
    "repository": "github.com/_AlexPersonalAccount/alpha-fork",
    "region": "UK",
    "tags": [
        "dev"
    ]
...
cluster: invalid value "live03333333333333" (does not satisfy strings.MaxRunes(16)):
    ...
    ./config-b.json:2:16
    ./schema.cue:4:32
repository: invalid value "github.com/_AlexPersonalAccount/alpha-fork" (out of bound =~"^source\\.company\\.example/"):
    ./schema.cue:6:15
    ./config-b.json:3:19
{{{end}}}
