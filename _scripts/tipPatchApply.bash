...
#
# tipPatchApply updates the working tree to be based on the tip of CUE. It first
# moves to a detached HEAD state and applies the diff found in $patchPath.
# The version of CUE available via cuelang.org/go@$CUE_COMMIT_REF is resolved, and
# The site configuration is overridden so that this version takes the place of
# all named CUEs (currently "latest", "prerelease", "tip" and "default").
# CUE_COMMIT_REF defaults to "master" - running the script with no arguments
# will use the tip of cue-lang/cue.
