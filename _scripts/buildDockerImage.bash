...
function tagAndPushContainerImage() {
    echo "Tagging local image $tag as $registryTag ..."
    docker tag $tag $registryTag
    echo "Pushing $registryTag to registry ..."
...
if docker inspect $tag > /dev/null 2>&1; then
    echo "local docker image $tag already exists; skipping build"
    # Only push the already-built container image if it doesn't exist in the remote registry.
    if [[ "${CI_PUSH_CONTAINER_IMAGE:-}" == "true" ]]; then
        if ! docker manifest inspect $registryTag >/dev/null 2>/dev/null; then
            tagAndPushContainerImage
        fi
    fi
	exit 0
fi
