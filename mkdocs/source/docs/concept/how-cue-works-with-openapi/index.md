...
```` { .text title="TERMINAL" data-copy="cue def schema.cue -o api.pet.yaml --out openapi+yaml" }
...
```` { .text title="TERMINAL" data-copy="cue import -p api api.pet.yaml" }
...
```` { .text title="TERMINAL" data-copy="cue vet -c api.pet.yaml jonathan.yml -d &#39;#Pet&#39;" }
