...
As further inspiration, here a more fully-formed example that demonstrate the
concepts and features that users of CUE find so compelling and powerful ...

### Elegantly Combining Schema, Policy, and Data Whilst Reducing Boilerplate
...
all function as you might expect and hope.
...
{{< code-tabs >}}
{{< code-tab name="site.cue" language="cue" area="top-left" >}}
package website
...
Whilst our site metadata is correctly specified, it does seem a little
*unwieldy*. There's quite a lot of duplicated information that distracts from
the important detail of each page - so-called *boilerplate*.
Let's use CUE to reduce it ...

We'll start by specifying a [default value](FIXME) for a page's publishing date,
its draft status, and its author:

{{< code-tabs >}}
{{< code-tab name="someDefaults.cue" language="cue" area="top-left" >}}
package website
...
#Page: {
	...
	isDraft: _ | *false
	author:  _ | *null
	... // An ellipsis avoids having to mention the other fields permitted inside #Page.
}
{{< /code-tab >}}{{< /code-tabs >}}

A quick `cue vet` verifies that adding these defaults hasn't broken anything,
and then we use `cue trim` to *automatically remove the unnecessary boilerplate
from our `site.cue` metadata file*, and `cue export` to confirm that the
boilerplate is still present in the data, where it's needed:

```text { title="TERMINAL" codeToCopy="Y3VlIHZldApjdWUgdHJpbQpjYXQgc2l0ZS5jdWUKY3VlIGV4cG9ydCAtLW91dCB5YW1s" }
$ cue vet
$ cue trim
$ cat site.cue
package website
...
Let's continue to reduce the boilerplate by updating our `someDefaults.cue`
file to use a pattern constraint to inject a [template](FIXME) into each member
of the `pages` struct.
This template will set up some defaults for the URL path and file path by
referencing the page's struct identifier:
...
Another `cue vet`, `cue trim`, and `cue export` show us how far we've managed
to reduce the `site.cue` metadata file:
