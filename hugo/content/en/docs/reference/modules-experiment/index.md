---
title: "Experimental modules support for CUE"
weight: 10
authors:
- rogpeppe
---
...
{{<info>}}
This document describes the experimental modules feature introduced in CUE FIXME(version).
{{</info>}}

## Introduction {#intro}

Modules are how CUE manages dependencies.
This document will be a detailed reference manual for CUE's module system
although it's a work in progress now.
...
A <dfn>module</dfn> is a collection of packages that are released, versioned, and distributed together. Modules are downloaded from
[OCI-compliant](https://github.com/opencontainers/distribution-spec/blob/main/spec.md)
artifact registries. This means that if you're deploying CUE to the
cloud, you can use the same distribution mechanism that you might be
using for Docker images to deploy your CUE configuration too.

A module is identified by a module path, which is declared in a `cue.mod/module.cue` file, together with information about the module’s dependencies. The module root directory is the directory that contains the `cue.mod` directory. The main module is the module containing the directory where the `cue` command is invoked.

Each package within a module is a collection of source files that are compiled together, usually all in the same directory.
...
Module paths are domain-name qualified: the expectation is that any modules you create should have
names that are inside domains or namespaces that you have control of, enabling modules from
different creators to live together without conflicts in the same registry.
...
A module path should describe both what the module does and where to find it. A module path consists of a root path and a major version suffix.

* The <dfn>root path</dfn> is the portion of the module path that identifies the OCI repository within a registry. All versions of a module are located in that same OCI repository.
* The major version suffix declares the major version of the module and is of the form `@v1` where the version `v1` here must match the major version of the full version it's been published as.
...
to the restrictions [documented there](https://github.com/opencontainers/distribution-spec/blob/main/spec.md#pulling-manifests).
To summarize:
...
No restriction is enforced on the length of module names, but for full compatibility,
the module path should be significantly under 128 characters in length.
...
has the path `example.com/mod@v1` at `v1.0.0`, it must have the path
`example.com/mod@v2` at version `v2.0.0`.

Major version suffixes implement the [_import compatibility
rule_](https://research.swtch.com/vgo-import):
...
Unlike in Go, major version suffixes are always required. The burden
...
build list, CUE will check whether `example.com/a` contains the
...
When the CUE looks up a new module for a package path, it checks the
...
prefix `github.com/acmecorp/`, such as `github.com/acmecorp/somemodule` which will be fetched from the `modules/github.com/acmecorp/somemodule` repository in the host
`registry.acme.example` at port 6000.
...
The module file schema comes in two forms: a non-strict version which
allows some laxity in specifying versions and module paths, and a strict
version that enforces canonical versions and module paths throughout.

The non-strict version can be turned into the strict version by running
`cue mod tidy`.

Here is the non-strict version:
...
The strict version of the schema further constrains the above schema as
follows:
...
module: "example.com/my/thing@v1"

deps: {
	"example.com/other/thing@v1": v: "v1.0.2"
	"example.com/new/thing@v2": v: "v2.3.4"
}
```

The `module.cue` file is designed to be human readable and machine writable. The
`cue` command will provide several subcommands that change `cue.mod/module.cue` files.
...
that can be changed by setting `$CUE_MODCACHE`.


## Authorization

The usual way to configure registry authorization information for custom OCI registries
is by setting them up in the `$HOME/.docker/config.json` file.
You can [use `docker login`](https://docs.docker.com/engine/reference/commandline/login/) to do this or [edit the file directly](https://www.flatcar.org/docs/latest/container-runtimes/registry-authentication/).
...
number. For example, `@v2` in `example.com/mod@v2`. See
