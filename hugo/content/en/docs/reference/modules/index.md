...
artifact registries. This means that if you're deploying CUE to the cloud,
...
compiled together, usually all in the same directory.  A package path
...
A module path should describe both what the module does and where to find it. A module path consists of a root path and a major version suffix, for example in the module path `myhost.example/foo@v0`,
the root path is `myhost.example.foo` and the major version suffix is `@v0`.
...
major version suffixes are always required. The burden
