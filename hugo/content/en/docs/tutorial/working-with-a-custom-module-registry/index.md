...
`cue.mod/module.cue` file declaring its module path - which is the path prefix
that must be used when importing packages from within the module.

We will describe using a custom registry in this tutorial, so there are fewer <!-- TODO: fewer /than/ what? -->
...
or a GitHub repository that you control. In our example we will assume that
...
to holding this tutorial's modules.
{{< /step >}}
