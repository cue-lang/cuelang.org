...
The project publishes powerful Go APIs that allow
[fine grained control over data validation workflows]({{< relref "docs/concept/how-cue-works-with-go" >}}),
and the `cue` command line tool that enables a wide range of data validation
tasks to be completed without writing any code.

This guide demonstrates some of the data validation capabilities of the `cue` command.
Because the command is written using the Go APIs, everything shown on this page
can also be achieved in code if your situation demands lower-level control.
