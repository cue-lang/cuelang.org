...
```text { title="TERMINAL" codeToCopy="Y3VlIHZldCAuIGFsZXguanNvbiBicnluLmpzb24gY2hhcmxpZS55YW1s" }
...
us get started quickly, performing partial validation of the most critical data
\- without having to spend time exhaustively documenting each and every field.
As we'll see shortly, this behaviour can be tightened up when required.
...
The `schema.cue` file is present from before. Let's create `policy.cue` as well:
...
separation of validation concerns into *any number of facets*, limited only by
...
allows each team to control its own constraints, with CUE taking responsibility
for combining them and checking that data validates against every facet at the
same time.
...
However, as you'll see in the following CUE, the language's
[standard library]({{< relref "docs/tour/packages/standard-library" >}})
provides many built-in validator functions. Here we use a pair of functions
that can validate string data in a more straightforward and clear way than
regular expressions.
...
- {{< linkto/related/reference "cli/cue-filetypes" >}}
  -- how to control which formats the `cue` tool processes
- {{< linkto/related/reference "cli/cue-inputs" >}}
  -- how the `cue` tools assembles its inputs, including its standard input stream
