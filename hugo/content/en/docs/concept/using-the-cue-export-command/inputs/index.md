...
providing its *import path* as part of an argument:
...
names of the files being used or add files to the package -- potentially
leading to errors due to missing configuration, if care is not taken to include
every file that's part of a package.
...
Because
[all valid JSON is also CUE]({{< relref "/docs/tour/basics/json-superset" >}}),
the standard input stream can contain JSON without requiring a `json:` qualifier.
<!-- TODO(jcm): Review if changes mentioned in https://cuelang.org/cl/1197464/comment/88110b0b_18176f2f/ happen. -->
