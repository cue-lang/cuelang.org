...
- If a dependency of the current module declares a module path that has the
  ...
  - if that directory contains a package then the package is located
    in that directory.
- The package contents are looked up in the `cue.mod/pkg`, `cue.mod/gen`, and
  `cue.mod/usr` directories. If a package is found in any of these three
  directories then the package content is unified from the relevant files
