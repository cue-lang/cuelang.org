...
Use of these directories is deprecated as they interact poorly with 
shared dependencies (those managed by the
[`cue mod`]({{< relref "docs/reference/command/cue-help-mod" >}}) command),
and are not visible if their containing module is used as a dependency.
...
1. If the import path (or some prefix of it) is defined in the `deps` field of
   [the `cue.mod/module.cue` file]({{< relref "docs/reference/modules" >}}#cue-mod-file)
...
2. The package contents are looked up, unconditionally, in the `cue.mod/pkg`,
   ...
   relevant files across all three directories.
