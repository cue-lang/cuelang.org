...
<!-- this is just for CL-previews, and needs removing before submission? -->
{{< caution >}}
...
The `example.cue` file contains two
[definitions]({{< relref "docs/tour/types/defs/" >}}) that we want to process:
`#Person` and `#Address`.
It also includes concrete data fields and a hidden field,
which we don't consider as schema. The data and hidden fields are included to
demonstrate that they are *not* walked by the code presented below.
{{< /step >}}
...
As you can see, this Go program is a very limited form of code generator,
taking each CUE definition and generating a Go struct type from it.
{{< /step >}}
