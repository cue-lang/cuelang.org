...
will implicitly check that each workflow's structure complies with the schema's
requirements.
...
`.github/workflows/` along with all the new files in `internal/ci/github/`.
...
changes. The use of a schema to check that your workflows comply with the form
required by GitHub Actions means that you will catch and fix mistakes in the
structure of your workflows earlier than before, without waiting for the slow
...
$ git add -p .github/workflows/ internal/ci/github/
[ ... ]
$ git commit -m "ci: added new release workflow"
```

## See also

- FIXME: Using CUE templates to DRY-up your GitHub Actions workflows
